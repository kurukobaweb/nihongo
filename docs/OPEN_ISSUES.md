...
> **目的: 未確定事項の唯一の管理台帳**
> **最終更新: 2026-08-16（引継ぎ時の実装・インフラ監査に基づく OI-113〜OI-119 の登録、および OI-021 への滞留 submission 論点の追記）**
...
| OI-119 | 仕様 | 推奨環境と検証環境 matrix の整合。特定商取引法に基づく表記が macOS 12以降および Microsoft Edge を推奨環境として掲げているが、T000-06-01 で確定した検証環境 matrix（`env-a`〜`env-d`）に含まれておらず、T013-09 の対象にもない。MediaRecorder の挙動はブラウザ差が確認されているため、推奨環境から外すか検証対象へ追加するかを確定する | 特商法表記, T000-06-01 環境 matrix, T013-09 | MVP | T013-09 着手前 | 中 |
...
| OI-021 | 運用 | 音声残存ファイル削除用 CleanupTempFilesJob の実行頻度および削除対象条件。即時削除失敗時の回復手段として、実行頻度だけでなく削除対象の経過時間条件も確定する。併せて、実行中ジョブと競合しない前提で、長時間 `pending` / `processing` に滞留した submission の音声ファイルの扱い（滞留判定の経過時間しきい値、submission を `failed` へ確定させるか否か、削除可否）を確定する | Scheduler, ストレージ管理, 音声一時ファイル削除 | MVP/R1 split | 実装前 | 中 |
...
| OI-114 | 性能 | 単一 VPS（3Core / 2GB / SSD 200GB）における Docker 構成のリソース妥当性。本番・開発の2環境を同一ホストで稼働させる前提で、Python 音声評価サービスと Queue Worker を追加した後の実メモリ使用量を計測し、プラン変更の要否を判断する。swap 2GB は設定済み。追加前の基準値は T001-06 の確認結果に記録 | ARCHITECTURE.md §14.1, Docker 構成, T001-07, T013-06, T013-07 | MVP/R1 split | Python コンテナ追加後の実測時 | 高 |
...
| OI-113 | 運用 | メール配信サービスの選定・契約・送信ドメイン認証。現状 `MAIL_MAILER=log` でメールが送信されず、メール認証・パスワードリセット・退会完了通知が機能しない。サービス選定、SPF / DKIM / DMARC 設定、送信上限、料金を確定する | ARCHITECTURE.md §12.1, T003-02, T015-02-01, `.env` | R1 | 本番構築前（DNS 反映に1〜2週を要するため早期着手） | 高 |
| OI-116 | 運用 | CI 導入の要否。`.github/` が存在せず自動テストの実行が人手依存である。全 Task に「テスト観点」が定義されている一方で実行の担保がない。MVP では ARCHITECTURE.md §1.2 によりスコープ外だが、本番運用開始後の回帰検知手段として要否を判断する | 開発フロー全体, ARCHITECTURE.md §1.2 | R1 | 本番構築前 | 中 |
| OI-118 | 運用 | サポート体制と問い合わせ窓口の方式。メールアドレス公開のみとするか問い合わせフォームを実装するかで T014-01 の実装範囲が変わる。対応時間、障害時の告知方法を含めて確定する | DESIGN.md §4.1 静的ページ群, T014-01, プライバシーポリシー第10条 | R1 | T014-01 着手前 | 中 |
...
| OI-115 | 非機能 | `evaluations.transcript` および `raw_azure_response` の保持期間。音声ファイルは即時削除する一方、文字起こしと Azure 生応答は無期限保存される非対称な設計になっている。プライバシーポリシーの記載内容と整合させ、保持期間および削除手段（退会時の hard delete 以外の削除経路の要否）を確定する。OI-107（500KB 超過時の保持方針）とは別論点として管理する | DB_SCHEMA.md evaluations, プライバシーポリシー, 退会フロー, T008-05 | MVP | 公開前 | 高 |
| OI-117 | 事業 | アクセス解析（Google Analytics 等）の導入要否。プライバシーポリシー第9条2項が「Google Analytics を利用しています」と記載しているが、実装には存在しない。導入するか、ポリシーの記載を削除するかを確定する | プライバシーポリシー第9条, フロントエンド実装, Cookie 同意の要否 | MVP/R1 split | 公開前 | 中 |
