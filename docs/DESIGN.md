...
| 項目 | 選択肢 | デフォルト |
|---|---|---|
| 出題方式（`question_format_preference`） | `single_prompt`（単体問題） / `two_choice`（2択） | `single_prompt` |
| タイマー表示方式（`timer_display_mode`） | `count_down`（カウントダウン） / `hidden`（非表示） | `count_down` |
...
* 出題方式は問題選択時の初期選択・絞り込みだけに使用し、問題形式、submission、採点条件を上書きしない
* タイマー表示方式にかかわらず、評価profile別の録音上限監視は常時有効とする

#### 保存先

設定項目の永続化方式は OI-023 で `user_learning_settings` テーブル方式に確定済みである。  
`users` JSONB方式は採用しない。  
historical T011-02で当時の5設定項目を保存する基盤を実装済みである。current設定は`question_format_preference` / `timer_display_mode`の2項目とし、historical実装との差はT011-03で補正する。
...
アクセス時は、未ログインユーザーを既存auth middleware / login導線に従ってログイン画面へ遷移させる。認証済みの一般ユーザー（`users.role = user`）はHTTP 403で拒否し、認証済みのadminだけ管理画面へ到達できる。
