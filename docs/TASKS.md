...
- 12〜17章は章番号順だけで実施順序を判断しない
- T017-02はT012前に行う課金なし音声提出E2Eであり、MVP全体完了判定ではない
- T012はT017-02完了後、T017-06前に実施するStripeタスク群として扱う
- T017-06はT012後に行うStripe E2Eとして扱う
- T017-10はT017-01〜T017-09完了後に行うMVP全体完了判定として最後に扱う
- Stripe、管理画面、設定保存先の永続化は、課金なし音声提出E2E成立後または該当OI確定後に進める
- 課金なし音声提出E2E前に必要な運用ログ確認は、音声提出経路に限定する
- Stripe Webhook受信 / 処理ログはT012-04 / T012-05側で扱い、T016-04には含めない
- 管理画面はT014-01 / T014-02をT012前でも実施可能なadmin入口確認とし、T014-03はOI-028確定後に限定する
- 依存タスク欄に複数IDがある場合、原則としてすべて完了してから着手する

| 観点 | 修正前の読まれ方 / 問題 | 修正後の扱い |
|---|---|---|
| T017-02 | T016-04経由でT012依存が混入する恐れ | T012前の課金なし音声提出E2E |
| T012 | T017全体後と誤読される恐れ | T017-02後、T017-06前 |
| T016-04 | 音声ログとWebhookログが混在 | 音声提出E2E前ログ確認基盤 |
| T017-06 | 17章内でT017-02と混同される恐れ | T012後のStripe E2E |
| T017-10 | T017-02と混同される恐れ | MVP全体完了判定 |
| T015-01 | ページ枠と料金 / 解約案内本文が混同される恐れ | ページ枠と確定文言反映を分離 |
...
12章はT017全体完了後ではなく、T017-02の課金なし音声提出E2E完了後に着手する。T012-01〜T012-06は、T017-06のStripe契約開始・解約・Webhook同期E2E前に必要なStripeタスク群として扱う。Webhook対象イベント一覧はOI-027確定前に固定しない。
...
- CodeX投入時の注意:
  ...
  - T012はT017-02後、T017-06前のStripeタスク群として扱う
...
- CodeX投入時の注意:
  ...
  - Stripe Webhook受信 / 処理ログはT012系タスクで扱う
  - T016-04の音声提出E2E前ログ確認基盤とは分離する
...
13章は画面枠と退会処理本体で実施タイミングが異なる。T013-01は退会確認画面のためT012前でも実施可能とし、T013-02はStripe解約予約を含むためT012-03依存を維持してT012後要素として扱う。T013-03はT013-02後に扱い、hard delete実行主体はOI-105に従う。
...
- テスト観点:
  ...
  - セッション削除
- CodeX投入時の注意:
  - Stripe解約予約を含むためT012-03依存を維持し、T012後要素として扱う
...
14章はadmin入口確認と管理機能範囲の確定を分けて扱う。T014-01 / T014-02はadmin権限と最小シェルのためT012前でも実施可能とし、T014-03はOI-028確定後に限定する。T014-03で契約状態閲覧などStripe関連機能を含める場合は、T012後の要素として扱う。
...
- CodeX投入時の注意:
  ...
  - 契約状態閲覧などStripe関連機能を含める場合はT012後の要素として扱う
...
- 実装内容:
  ...
  - お問い合わせページ枠
  - 料金 / 解約案内ページ枠
  - 料金 / 解約案内の本文反映は、T012-03完了後または確定済み原稿がある場合に限定する
- 実装してはいけないこと:
  - 法務文言を作成・確定しない
...
- CodeX投入時の注意:
  ...
  - 料金 / 解約案内の本文反映はT012-03完了後または確定済み原稿がある場合に限定する
...
- 実装してはいけないこと:
  ...
  - Stripe Webhook受信 / 処理ログはT012-04 / T012-05側で扱う
...
17章は実施タイミングが異なるE2Eを含む章であり、章全体を一括で最後に実施するものではない。T017-02はT012前の課金なし音声提出E2E、T017-06はT012後のStripe E2E、T017-10はT017-01〜T017-09完了後のMVP全体完了判定として扱う。T017-02をMVP全体完了判定として扱わない。
...
- CodeX投入時の注意:
  ...
  - T012前の課金なし音声提出E2Eとして扱う
  - 依存するT016-04は、Stripeを含まない音声提出E2E前ログ確認基盤を指す
...
- CodeX投入時の注意:
  - Stripe後半タスクとして実施する
  - T012完了後のStripe E2Eとして扱い、T017-02の課金なし音声提出E2Eとは分ける
...
- CodeX投入時の注意:
  - 実装修正ではなく判定レビューとして扱う
  - T017-01〜T017-09完了後の最後のMVP全体完了判定として扱う
