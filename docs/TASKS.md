...
- T013-02は新T014のStripeタスク前に行う課金なし音声提出E2Eであり、MVP全体完了判定ではない
...
| 観点 | 修正前の読まれ方 / 問題 | 修正後の扱い |
|---|---|---|
| 旧T017-02 / 新T013-02 | 旧T016-04経由で旧T012依存が混入する恐れ | 新T014前の課金なし音声提出E2E |
| 旧T012 / 新T014 | 旧T017全体後と誤読される恐れ | 新T013-02後、T014-08前 |
| 旧T016-04 / 新T012-04 | 音声ログとWebhookログが混在 | 音声提出E2E前ログ確認基盤 |
| 旧T017-06 / 新T014-08 | 旧17章内で課金なしE2Eと混同される恐れ | Stripeタスク後のStripe E2E |
| 旧T017-10 / 新T017-01 | 課金なしE2Eと混同される恐れ | MVP全体完了判定 |
| 旧T015-01 / 新T014-01 | ページ枠と料金 / 解約案内本文が混同される恐れ | ページ枠と確定文言反映を分離 |
...
- 実装PR:
  - #26
- 実装commit:
  - 293f0d660b408507d7fc47caebbf7aac54adf57b
- merge commit:
  - 0cf9dd05c49c58343aaad8ce25e5c58258d3d234
- 確認内容:
  - CleanupTempFilesJobを追加
  - CleanupTempFilesCommandを追加
  - TemporaryAudioFileCleanerを追加
...
## 旧ID→新ID対応表（T012〜T017再整理）

| 旧ID | 新ID | タスク名 | 備考 |
|---|---|---|---|
| T016-01 | T012-01 | CleanupTempFilesJob 実装 | PR #26で実装済み。新T012-01に完了反映。 |
| T016-02 | T012-02 | Queue Worker運用確認タスク | 課金なしE2E前の運用準備へ移動 |
| T016-03 | T012-03 | Feature Flag反映確認タスク | 課金なしE2E前の運用準備へ移動 |
| T016-04 | T012-04 | 音声提出E2E前ログ確認基盤 | Stripe Webhookログは新T014側で扱う |
| T017-01 | T013-01 | 認証E2Eテスト | 基本結合テストへ移動 |
| T017-02 | T013-02 | 課金なし音声提出E2Eテスト | 新T014のStripeタスク前の中間E2E |
| T017-03 | T013-03 | 422認識不可E2Eテスト | 課金なしE2Eの異常系 |
| T017-04 | T013-04 | Feature Flag OFF結果画面テスト | 課金なしE2E後の結果画面確認 |
| T017-05 | T013-05 | 音声ファイル削除テスト | CleanupTempFilesJob確認を含む |
| T017-09 | T013-06 | 3秒ポーリング簡易負荷確認 | 課金なしE2E後の簡易負荷確認 |
| T015-01 | T014-01 | 法務・情報系静的ページ作成 | Stripe契約導線前のページ枠 |
| T012-01 | T014-02 | Laravel Cashier導入 | Stripeタスク群へ移動 |
| T012-02 | T014-03 | Checkout / 顧客作成導線実装 | 法務・料金ページ枠後に実施 |
| T012-03 | T014-04 | Billing Portal / 契約状態表示 / 解約導線実装 | 契約状態同期はWebhook処理と分離 |
| T012-04 | T014-05 | Webhook受信・署名検証の土台実装 | OI-027未確定イベントを固定しない |
| T012-05 | T014-06 | Webhook Queue処理基盤実装 | Webhook処理ログはStripe文脈で扱う |
| T012-06 | T014-07 | OI-027確定後のWebhookイベント反映 | OI-027確定後に限定 |
| T017-06 | T014-08 | Stripe契約開始・解約・Webhook同期テスト | Stripeタスク後のE2E |
| T013-01 | T015-01 | 退会確認画面実装 | 退会フローへ維持 |
| T013-02 | T015-02 | 退会処理 soft delete 実装 | 新T014-04依存へ更新 |
| T013-03 | T015-03 | 30日後hard delete候補抽出 | OI-105未確定を維持 |
| T017-07 | T015-04 | 退会フローテスト | 退会章の結合テストへ移動 |
| T014-01 | T016-01 | admin middleware 実装 | 管理入口章へ移動 |
| T014-02 | T016-02 | 管理画面最小シェル実装 | 管理入口章へ移動 |
| T014-03 | T016-03 | OI-028確定後の管理画面機能分割 | OI-028確定後に限定 |
| T017-08 | T016-04 | 管理画面admin権限テスト | 管理入口の権限テスト |
| T017-10 | T017-01 | MVP完了判定レビュー | 最終確認へ移動 |

---

# 13. 課金なしE2E・基本結合テスト

13章は、新T014のStripeタスクに進む前の課金なしE2Eと基本結合テストを扱う。T013-02は課金なし音声提出E2Eであり、Stripe、退会、管理画面、運用確認を含むMVP全体完了判定とは分ける。
...
- 種別: テスト
- 目的:
  - 新T014のStripeタスク前に、最初の大マイルストーンである課金なし音声提出E2Eを確認する
- 参照仕様書:
  - `ARCHITECTURE.md`
  - `DESIGN.md`
  - `DB_SCHEMA.md`
...
- CodeX投入時の注意:
  ...
  - 新T014のStripeタスク前に実施する課金なし音声提出E2Eとして扱う
...
- 実装内容:
  ...
  - Checkout Session作成
  - サブスクリプション管理画面からの導線
  - Checkout導線に必要な法務・料金ページリンク先が存在することを確認する
- 実装してはいけないこと:
  - 年額プランを実装しない
  - PDF領収書をアプリ内生成しない
- 完了条件:
  - Checkoutへ遷移できる
  - Checkout導線から必要な法務・料金ページへ辿れる
- テスト観点:
  - 未ログイン
  - 既存顧客
  - Checkout失敗
...
- 実装内容:
  ...
  - 解約導線
  - Webhook同期
  - 課金導線上の利用規約 / プライバシーポリシー / 特定商取引法 / 料金・解約案内ページへのリンク確認
- 実装してはいけないこと:
  - OI-027未確定のイベントをテスト前提にしない
- 完了条件:
  - 課金関連ページの導線が存在し、Stripe契約開始・解約・Webhook同期が成立する
- テスト観点:
  - 契約開始
  - 解約予約
  - Webhook重複
...
- CodeX投入時の注意:
  - Stripe後半タスクとして実施する
  - T014完了後のStripe E2Eとして扱い、T013-02の課金なし音声提出E2Eとは分ける
...
- 完了条件:
  - 退会確認画面、Stripe解約予約、音声削除、sessions削除、soft delete、hard delete候補抽出までが仕様通り確認できる
- テスト観点:
  - 契約あり
  - 音声残存あり
  - ログイン不可
